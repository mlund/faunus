...
void GCtitrate::init(vector<particle> &p, group &mobile) {
  for (int i=0; i<p.size(); i++)    //search for titrateable sites
    if ( (*spc).d[p[i].id].pka !=0 ) {
      sites.push_back(i);
      p[i].charge = spc->d[p[i].id].p.charge; // deprotonate
...
//returns number a random, titrateable
//site in particle vector
short int GCtitrate::random(vector<short int> &iv) {
  short int i = rand() % iv.size();     //and pick randomly
  return iv.at(i);
}

//Returns protonation status of particle i in particle vector
  GCtitrate::keywords GCtitrate::status(vector<particle> &p, short int i) {
    if ( p[i].charge > (*spc).d[p[i].id].p.charge )
      return PROTONATED;
    return DEPROTONATED;
  }

  GCtitrate::action GCtitrate::takeFromBulk(vector<particle> &p, short int i, short int j) {
    if (j==-1)                               //if not specified..
      j=random(protons);                     //..pick a random proton
  ...
  GCtitrate::action GCtitrate::moveToBulk(vector<particle> &p, short int i, short int j) {
    if (j==-1)             // if not specified..
      j=random(neutrons);  // ..pick random neutron
    neutrons.erase(remove(neutrons.begin(), neutrons.end(), j)); //del a neutron
  ...
  GCtitrate::action GCtitrate::exchange(vector<particle> &p) {
    int i=random( sites );        // pick a random, titrateable particle
    if (status(p,i)==PROTONATED)
      return moveToBulk(p,i);
    else
  ...
  GCtitrate::action GCtitrate::exchange(vector<particle> &p, action &a) {
    if (status(p, a.site)==PROTONATED)
      return moveToBulk(p, a.site, a.proton);
    else
      return takeFromBulk(p, a.site, a.proton);
...
double GCtitrate::energy(vector<particle> &p,
    double du, action &a) {
  int i=p[a.site].id;
  if (a.action==PROTONATED)
    return du+( log(10.)*( ph - spc->d[i].pka ) );
...
void GCtitrate::infos() {
  cout << "# Titrateable sites   = " << sites.size() << endl
    << "# Protons             = " << protons.size() << endl
    << "# pH                  = " << ph << endl ;
}

double GCtitrate::sumsites() {
  double Q=0;
  for (int i=0; i<sites.size(); i++)
    Q+= q[i].avg();
  return Q;
...
void GCtitrate::samplesites(vector<particle> &p) {
  for (int i=0; i<sites.size(); i++)
    q[i] += p[sites[i]].charge;
  nprot+=protons.size();
}

void GCtitrate::showsites(vector<particle> &p) {
  cout << "# --- AVERAGE SITE CHARGES ---------------------\n";
  for (int i=0; i<spc->d.size(); i++)
    if (spc->d[i].pka!=0) {
      cout << spc->d[i].name << ": " << setiosflags(ios::fixed);
 ...
 * to disk, so as to include the partial charges of the system.
 */
double GCtitrate::applycharges(vector<particle> &p) {
  for (int i=0; i<sites.size(); i++)
    p[sites[i]].charge = q[i].avg();
