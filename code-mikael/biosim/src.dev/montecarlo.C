...
int montecarlo::speed() { return (accepted+rejected)/(time(0)-time_i); }
...
void montecarlo::showStatus(int macroCnt) {
  progress = static_cast<double>(macroCnt)/macroSteps;
  ...
  cout << "# " << progress*100 << "% ETA: " << finishStr;
};
...
void montecarlo::adjust_dp(trialtype type, double &dp, double min, double max) {
  ...
  if (dp<=0) dp=0.1; // we do not allow negative dp values.
