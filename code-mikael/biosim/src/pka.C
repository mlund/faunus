  ...
  for (int macroCnt=1; macroCnt<=mc.macroSteps; macroCnt++) {
	...
	for (int cnt=0; cnt<tit.sites.size(); cnt++) {
	  ...
	  if (rc==true) {
        ...
        mux+=s.p[g[PROT].dipv].x;
        muy+=s.p[g[PROT].dipv].y;
        muz+=s.p[g[PROT].dipv].z;
    ...
    cout << "# System energy (kT)       = " << usys << endl
         ...
         << "#   dip. mom. vector       = " << s.p[g[PROT].dipv] << endl
         << "#   avg. dip. mom vector   = " << mux.avg() << " " << muy.avg() << " " << muz.avg() << endl
         << "#   avg. dip. mom vec. org = "
                      << mux.avg()+orgcm.x << " " << muy.avg()+orgcm.y << " " << muz.avg()+orgcm.z << endl
