...
class trial {
 private:
  void add(bool);
 public:
  unsigned long long int accepted,rejected,cnt;
  string name;
  trial();
...
private:
  double progress;
  unsigned long long int accepted, rejected, energy_rejected, hc_rejected, total;
  int elapsed_time, remaining;
  time_t rawtime;
  struct tm *timeinfo;

public:
  enum trialtype {ION=0, ROTATE, TRANSLATE, TITRATE, MONOMER, CLUSTER, SITE, ENDENUM};
