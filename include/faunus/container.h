...
namespace Faunus {
  ...
  class space {
    ...
    public:
      ...
      double charge(const point &, double);           //!< Sum all charges within a spherical region
      int count(unsigned char, const point&,double);  //!< Count space of "type" within a sphere

      virtual bool clash(const particle &, const particle &); //!< Overlap between two space
