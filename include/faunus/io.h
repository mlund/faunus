...
namespace Faunus {
  ...
  namespace IO {
    ...
    inline bool writeFile(string file, string s,
      std::ios_base::openmode mode=std::ios_base::app) { // append <-- joaoh
  ...
  class FormatPQR {
    ...
    public:
      template<class Tpvec, class Tvec=Point>
		...
		p_i.x() + (len.x() * 0.5), p_i.y() + (len.y() * 0.5), p_i.z() + (len.z() * 0.5), p_i.charge, p_i.radius ); // move particles inside the box <-- joaoh
            o << buf;
    ...
    public:
      ...
      template<class Tparticle, class Talloc>
        bool save(const string &file, std::vector<Tparticle,Talloc> &p) {
          int nres=1, natom=1;
          char buf[79];
          double halflen=len/20; // correct halflen from A to nm <-- joaoh
          std::ostringstream o;
          ...
          if (len>0)
            o << len/10 << " " << len/10 << " " << len/10 << std::endl; // correct box side length from A to nm <-- joaoh
          return IO::writeFile(file, o.str());
        }
      ...
      template<class Tpoint, class Talloc>
        bool save(const string &file, const std::vector<Tpoint,Talloc> &p) {
	      ...
	      x[i][0] = (pi.x() * 0.1) + (xdbox[0][0] * 0.5);      // AA->nm
              x[i][1] = (pi.y() * 0.1) + (xdbox[1][1] * 0.5);      // move particles inside the box <-- joaoh
