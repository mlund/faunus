...
#pragma omp parallel for reduction (+:du) schedule (dynamic)
    ...
    bool AtomTracker<Tspace>::insert( const Tparticle &a, Tindex index )
    {
        ...
        for ( auto &m : map )    // loop over all maps
            for ( auto &i : m.second.index ) // and their particle index
                if ( i >= index )
                    i++; // push forward particles beyond inserted particle
    ...
    template<class Tspace>
    void GrandCanonicalSalt<Tspace>::randomIonPair( typename Tparticle::Tid &id_cation,
                                                    typename Tparticle::Tid &id_anion )
    {
        do
            id_anion = tracker.randomAtomType();
        while ( map[id_anion].p.charge >= 0 );
        do
            id_cation = tracker.randomAtomType();
    ...
    void GrandCanonicalSalt<Tspace>::_trialMove()
    {
        ...
        switch (rand() % 2)
        {
            case 0:trial_insert.reserve(Na + Nb);
                do
                ...
                for ( auto &p : trial_insert )
                    spc->geo.randompos(p);
                break;
            case 1:trial_delete.reserve(Na + Nb);
