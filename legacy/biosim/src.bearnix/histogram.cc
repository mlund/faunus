...
void histogram::show(vector<histogram::data> &d,
  ...
  int min=int(minvalue/width); //first hist to show
  int max=int(maxvalue/width); //last hist to show
...
bool histogram::save(histogram::data &d, string file) {
  ...
  cout <<"!!! Histogram "<<d.name<<" could not be saved!!!\n"
       <<"!!! DATA LOST !!!"<<endl;
