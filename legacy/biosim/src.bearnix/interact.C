...
double interact::energy(vector<particle> &p, group &g1, group &g2) {
  ...
  if (ilen>g1.cm) ilen=g1.cm;  //avoid ghost particles
  if (jlen>g2.cm) jlen=g2.cm;  //such as "cm" and "dipv"
...
double interact::energy_vdw(vector<particle> &p, group &g1, group &g2) {
  ...
  unsigned int jlen=g2.end+1;
  if (ilen>g1.cm) ilen=g1.cm; // avoid ghost particles
  if (jlen>g2.cm) jlen=g2.cm; // such as "cm" and "dipv" (see group.h)
  if (g1.vdw==true && g2.vdw==true && vdw!=0)
    for (int i=g1.beg; i<ilen; i++)
      for (int j=g2.beg; j<jlen; j++) {
        r2=p[i].sqdist(p[j]);
...
//Metropolis test for GC tit. du must be in units of kT!
bool interact::metropolis(double du, double Pref) {
  if (Pref*exp(-du) < 1)
    if ( slmp.random_one()>Pref*exp(-du) )
      return false;
  return true;
};
//Metropolit test for salt flow, du and chemical pot. must 
//be in units of kT!
bool interact::metropolis(double du, double catPot, int flag, double density) {
  if (flag == 1) { //1=addition of a saltpair
    du-=catPot;
    if (exp(-du)/density< 1)
      if (slmp.random_one()>exp(-du)/density)
        return false;
    return true;  
  }else{
    du+=catPot;
