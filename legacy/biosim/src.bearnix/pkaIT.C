...
int main(int argc, char* argv[] ) {
  ...
  if (GCtit==true) {
    ...
    catPot = par.getDbl("catPot", 0.);

};
  double catPotinit =catPot;
  /**************************************
...
//  imd.wait_for_connection();
  ...
  for (int macroCnt=1; macroCnt<=mc.macroSteps; macroCnt++) {
	...
	for (int cnt=0; cnt<tit.sites.size(); cnt++) {
	  ...
	  du=unew-uold;
          if (scwidom==true && GCtit==true) //If true, Semi Grand Canonical titration
            rc=pot.metropolis( tit.energy(s.trial, du, catPot, t) );
          else
	...
	double x2;
      ...
      if (slump.random_one()>0.8 && scwidom==true){
        ...
        if (GCtitEQ==true)
          catPot=scwidomCAT.exessChempot();  //Update the excess chemical potential
      }                                     //for Grand Canonical titration
    ...
    cout << "# Excess.chem.pot Cation   = " << scwidomCAT.exessChempot() << endl
         ...
         << "# Diffrence in present and\n"
         << "# init. Cation excess      = " << catPot-catPotinit << endl;
         };
  ...
  //Update the inputfile so it matches the .corr file (as it should)

  cout << s.count(g[SALT], -1);
  par.updateInp(g[SALT].end-g[SALT].beg+1-s.count(g[SALT], -1), s.count(g[SALT], -1), inputName);
  return 0;
};
