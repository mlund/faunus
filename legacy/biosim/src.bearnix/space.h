...
public:
  ...
  group insert_salt(int, double, double, cell::cell &);

  int push_back(particle &);    //add particle to both "p" and "trial"
  int trialpush_back(particle &); //add a particle to "trial"
  ...
  void compoChangeIn(vector<particle> &, group &, keys=TRIALMOVE);
  vector<int> compoChangeOut(group &, keys=TRIALMOVE);
  ...
  int trialcount(group &, double);    ///< Count charges
