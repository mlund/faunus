...
int main(int argc, char* argv[] ) {
  ...
  ***********************************/
  ...
  //INITIAL ENERGY
  double utot = pot.energy_dh(s.p, g[P1], g[P2]); //initial energy
  ...
  for (int macroCnt=1; macroCnt<=mc.macroSteps; macroCnt++) {
      ...
      if (coll==false)
        if (z < 2*cell_r)
          if (col.overlap(s.trial, g[P1], g[P2])==false) {
            uold = pot.energy_dh(s.p, g[P1],g[P2]);
            unew = pot.energy_dh(s.trial,g[P1],g[P2]);
            du=unew-uold;
    ...
    double usys = pot.energy_dh(s.p, g[P1], g[P2]);
