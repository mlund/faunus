...
double space::charge(group &g) {
  double z=0;
  for (unsigned int i=g.beg; i<g.end+1; i++)
    z+=p[i].charge;
  g.charge=z;
  return z;
};
...
double space::radius(group &g, point &origo, keys k) {
  ...
  switch (k) {
    case MAX :
      for (unsigned int i=g.beg; i<=g.end; i++) {
...
group space::sort(group &g) {
  ...
  int j=0;
  vector<particle> s(0);
  for (unsigned int i=g.beg; i<=g.end; i++) //find charges
    if (p[i].charge != 0)
      s.push_back(p[i]);

  out.end = out.beg + s.size() - 1; //end of charges
  
  for (unsigned int i=g.beg; i<=g.end; i++) //find neutral
    if (p[i].charge == 0) s.push_back(p[i]);
  for (unsigned int i=g.beg; i<=g.end; i++) //copy into particle vector
    p[i]=s[j++];
...
void space::undo(group &g, keys k) {
  int ilen=g.end+1;
  if (k==ALL)
    for (unsigned int i=g.beg; i<ilen; i++)
      trial[i]=p[i];
  else
    for (unsigned int i=g.beg; i<=g.end; i++) {
      trial[i].x = p[i].x;
...
group space::insert_species(int n, particle p, cell &c) {
  insert_salt(n, p.charge, p.radius, c, p.id);
};
