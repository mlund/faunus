...
\begin{center}
\Huge
\textbf{A handy ``Lathund''\\ for MacOS X \& Linux\footnote{To a lesser extent.}}\\
\normalsize
\vspace{0.5cm}
\emph{By Mikael Lund, Bj\"orn Persson. Last updated \today}
\end{center}
...
\subsection{Macports}
\begin{itemize}
\item \verb"port search keyword" -- search for a package.
\item \verb+port variants packagename+ -- show \emph{variants} of a package.
\item \verb"sudo port install packagename" -- install a package.
\item \verb"sudo port install emacs +x11" -- install emacs variant with X11 support.
\item \verb"sudo port -d selfupdate" -- update entire package system (do this only occasionally).
\end{itemize}
