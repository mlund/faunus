...
\subsubsection{\label{sec:macports}Macports}
\begin{table}[h!]
\center
\begin{tabular}{ll}\hline\hline
...
\verb"sudo port -d selfupdate" & update entire package system (do this occasionally)\\\hline
\end{tabular}\end{table}
%
Useful packages are: grace gnuplot povray sshfs vim aspell wget gawk tetex findutils xmgr subversion sshfs.
...
Or just VMD is a very impressive program for molecular visualization/analysis and despite the name, it works nicely with MC. A speciel ``feature'' of the mac version is that the command line tool is buried. Here are two ways to surface it:
\begin{itemize}
...
\noindent Make sure to specify the compiler in the Makefile -- in particular parallel execution works, as of now, much better on Intels C++ compiler than with the GNU compiler.
...
\subsection{Xmgr}
I have made a MacPorts (\ref{sec:macports}) port of this program -- simply install it with \verb+sudo port install xmgr+.
Alternatively, a native MacOS package is available:\\

\url{http://idisk.mac.com/mlund/Public/Software/Xmgr.pkg.zip}\\

\noindent Note that the development of Xmgr was discontinued in 1998; consider \verb"Grace", its superior descendant.
...
\subsection{SSH Options for connections to mimer}
Add the following to \verb"~/.ssh/config" to allow X tunelling, start a proxy server (see Section~\ref{sec:proxy}) and forward local Subversion traffic to gloin:
\begin{verbatim}
Host mimer
  DynamicForward 9999
  ForwardX11 yes
  Compression yes
  LocalForward 8910 gloin:22
\end{verbatim}
