...
/**
 * Base class for calculating single value property between two groups
 */
class GroupGroupProperty {
  public:
    virtual double value(const Group& group1, const Group& group2) const = 0;
    virtual ~GroupGroupProperty() = default;
    virtual void to_json(json &j) const;
};

/**
 * @brief Nonbonded energy between groups
 */
class GroupGroupEnergy : public GroupGroupProperty {
  private:
    Energy::Hamiltonian& hamiltonian;

  public:
    double value(const Group& group1, const Group& group2) const override;
    GroupGroupEnergy(Energy::Hamiltonian& hamiltonian);
};
 ...
 * The output can be filtered to match a set of pre-defined
 * criteria which currently are:
 *
 * - `smaller_than`
 * - `absolute_larger_than`
 *
 * The possible properties are: `energy`
 */
class GroupMatrixAnalysis : public Analysisbase {
  private:
    const Space& spc;
    std::function<bool(double)> include_value;                  //!< Used to filter values generated by `policy`
    ...
    std::unique_ptr<std::ostream> stream;                       //!< Pair matrix output stream
    ...
    void setProperty(const json& j, Energy::Hamiltonian& hamiltonian);

  public:
    GroupMatrixAnalysis(const json& j, const Space& spc, Energy::Hamiltonian& pot);
    virtual ~GroupMatrixAnalysis() = default;
};
