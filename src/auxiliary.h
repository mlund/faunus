...
#pragma GCC diagnostic ignored "-Wstrict-aliasing"
     ...
     * two (more precise) iterations. Conditionals are evaluated at compile time.
     *
...
#pragma GCC diagnostic pop
     ...
     * @tparam n Maximum order to evaluate
     * @tparam use_table Use lookup table for 1+1/i? Default = false
     * @todo Benchmark `use_table`
     */
    template <typename T, std::size_t n, bool use_table = false> class Legendre {
      private:
        std::array<T, n + 1> y; //!< Lookup table for 1+1/i (overkill?)
        std::array<T, n + 1> P; //!< Legendre terms stored here
      public:
        Legendre() {
            P[0] = 1.0;
