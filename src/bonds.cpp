...
bool BondData::hasEnergyFunction() const
{
    return energyFunc != nullptr;
}

bool BondData::hasForceFunction() const
{
    return forceFunc != nullptr;
}

BondData::BondData(const std::vector<int>& indices)
    : indices(indices)
{
}

HarmonicBond::HarmonicBond(double force_constant, double equilibrium_distance,
                           const std::vector<int>& indices)
    : StretchData(indices)
    , half_force_constant(force_constant / 2.0)
...
{
    half_force_constant =
        j.at("k").get<double>() * 1.0_kJmol / std::pow(1.0_angstrom, 2) / 2.0; // k
...
std::shared_ptr<BondData> HarmonicBond::clone() const
{
    return std::make_shared<HarmonicBond>(*this);
}

BondData::Variant HarmonicBond::type() const
{
    return BondData::Variant::HARMONIC;
}
...
void HarmonicBond::setEnergyFunction(const ParticleVector& particles)
{
    energyFunc = [&](Geometry::DistanceFunction calculateDistance) -> double {
        ...
        const auto distance =
            equilibrium_distance - calculateDistance(particle1.pos, particle2.pos).norm();
    ...
    forceFunc = [&](Geometry::DistanceFunction calculateDistance) -> std::vector<IndexAndForce> {
        ...
        auto force =
            2.0 * half_force_constant * (equilibrium_distance - distance) * distance_vec / distance;
...
FENEBond::FENEBond(double force_constant, double max_distance, const std::vector<int>& indices)
    : StretchData(indices)
    , half_force_constant(0.5 * force_constant)
    , max_squared_distance(max_distance * max_distance)
{
}

std::shared_ptr<BondData> FENEBond::clone() const
{
    return std::make_shared<FENEBond>(*this);
}

BondData::Variant FENEBond::type() const
{
    return BondData::Variant::FENE;
}
...
void FENEBond::setEnergyFunction(const ParticleVector& particles)
{
    energyFunc = [&](Geometry::DistanceFunction calcDistance) {
        const auto squared_distance =
            calcDistance(particles[indices[0]].pos, particles[indices[1]].pos).squaredNorm();
        if (squared_distance >= max_squared_distance) {
            return pc::infty;
        }
        return -half_force_constant * max_squared_distance *
               std::log(1.0 - squared_distance / max_squared_distance);
    };
    forceFunc = [&](Geometry::DistanceFunction distance) -> std::vector<IndexAndForce> {
        ...
        if (squared_distance >= max_squared_distance) {
            throw std::runtime_error(
                "Fene potential: Force undefined for distances greater than rmax.");
        };
        const auto force_magnitude = -2.0 * half_force_constant * ba.norm() /
                                     (1.0 - squared_distance / max_squared_distance);
...
std::shared_ptr<BondData> FENEWCABond::clone() const
{
    return std::make_shared<FENEWCABond>(*this);
}

BondData::Variant FENEWCABond::type() const
{
    return BondData::Variant::FENEWCA;
}
...
void FENEWCABond::setEnergyFunction(const ParticleVector& particles)
{
    energyFunc = [&](Geometry::DistanceFunction calculateDistance) {
        double wca = 0.0;
        constexpr auto two_to_the_power_of_two_sixths =
            1.01944064370214482816981563263103378007648819; // 2^((1/6)^2)
        const auto squared_distance =
        ...
        return -half_force_constant * max_distance_squared *
                   std::log(1.0 - squared_distance / max_distance_squared) +
               wca;
    };
    forceFunc = [&](Geometry::DistanceFunction calculateDistance) -> std::vector<IndexAndForce> {
        const Point ba =
            calculateDistance(particles[indices[0]].pos, particles[indices[1]].pos); // b->a
        const auto distance = ba.norm();
        const auto squared_distance = distance * distance;
        if (squared_distance >= max_distance_squared) {
            throw std::runtime_error(
                "Fene+WCA potential: Force undefined for distances greater than rmax.");
        }
        ...
        const auto force_magnitude = -(2.0 * half_force_constant * distance /
                                           (1.0 - squared_distance / max_distance_squared) +
                                       wca_force);
        Point force0 = force_magnitude * ba.normalized();
        Point force1 = -force0;
        return {{indices[0], force0}, {indices[1], force1}};
...
{
    j = {{"k", 2 * half_force_constant / (1.0_kJmol / std::pow(1.0_rad, 2))},
         {"aeq", equilibrium_angle / 1.0_deg}};
    roundJSON(j, 6);
}

HarmonicTorsion::HarmonicTorsion(double force_constant, double equilibrium_angle,
                                 const std::vector<int>& indices)
    : TorsionData(indices)
    , half_force_constant(force_constant / 2.0)
...
BondData::Variant HarmonicTorsion::type() const
{
    return BondData::Variant::HARMONIC_TORSION;
}

std::shared_ptr<BondData> HarmonicTorsion::clone() const
{
...
void HarmonicTorsion::setEnergyFunction(const ParticleVector& particles)
{
    energyFunc = [&](Geometry::DistanceFunction calculateDistance) -> double {
        const auto vec1 =
            calculateDistance(particles[indices[0]].pos, particles[indices[1]].pos).normalized();
        const auto vec2 =
            calculateDistance(particles[indices[2]].pos, particles[indices[1]].pos).normalized();
    ...
    forceFunc = [&](Geometry::DistanceFunction calculateDistance) -> std::vector<IndexAndForce> {
        if constexpr (false) {
            ...
            const auto inverse_sine_angle =
                1.0 / std::sqrt(std::fabs(1.0 - cosine_angle * cosine_angle));
            const auto angle = std::acos(cosine_angle);
            const auto prefactor =
                2.0 * half_force_constant * (angle - equilibrium_angle) * inverse_sine_angle;
            ...
            const Point ba =
                calculateDistance(particles[indices[0]].pos, particles[indices[1]].pos); // b->a
            const Point bc =
                calculateDistance(particles[indices[2]].pos, particles[indices[1]].pos); // b->c
...
{
    j = {{"k", 2.0 * half_force_constant / 1.0_kJmol},
         {"aeq", std::acos(cosine_equilibrium_angle) / 1.0_deg}};
}

GromosTorsion::GromosTorsion(double force_constant, double cosine_equilibrium_angle,
                             const std::vector<int>& indices)
    : TorsionData(indices)
    , half_force_constant(0.5 * force_constant)
...
BondData::Variant GromosTorsion::type() const
{
    return BondData::Variant::GROMOS_TORSION;
}

std::shared_ptr<BondData> GromosTorsion::clone() const
{
...
void GromosTorsion::setEnergyFunction(const ParticleVector& particles)
{
    ...
    forceFunc = [&](Geometry::DistanceFunction distance) -> std::vector<IndexAndForce> {
        ...
        const auto angle = std::acos(cosine_angle);
        const auto force_magnitude = -2.0 * half_force_constant * std::sin(angle) *
                                     (cosine_angle - cosine_equilibrium_angle);
        const Point plane_abc = ba.cross(bc).eval();
        Point force0 = force_magnitude * inverse_norm_ba * ba.cross(plane_abc).normalized();
        Point force2 = force_magnitude * inverse_norm_bc * -bc.cross(plane_abc).normalized();
        Point force1 = -(force0 + force2); // Newton's third law
...
int PeriodicDihedral::numindex() const
{
    return 4;
}

std::shared_ptr<BondData> PeriodicDihedral::clone() const
{
...
BondData::Variant PeriodicDihedral::type() const
{
    return BondData::Variant::PERIODIC_DIHEDRAL;
}
...
void PeriodicDihedral::setEnergyFunction(const ParticleVector& particles)
{
    ...
    energyFunc = [&](Geometry::DistanceFunction distance) {
        ...
        const auto dihedral_angle = std::atan2((normal_abc.cross(normal_bcd)).dot(bc) / bc.norm(),
                                               normal_abc.dot(normal_bcd));
    ...
    forceFunc = [&](Geometry::DistanceFunction distance) -> std::vector<IndexAndForce> {
        ...
        const Point ortho_normalized_abc =
            -normal_abc.normalized(); // normalized vector orthogonal to the plane abc.
        const Point ortho_normalized_bcd =
            cd.cross(-bc).normalized(); // normalized vector orthogonal to the plane bcd.
        ...
        const Point torque_c =
            -(bc_midpoint.cross(force_d) + 0.5 * cd.cross(force_d) - 0.5 * ab.cross(force_a));
...
{
    j = {{"k", 2 * half_force_constant / (1.0_kJmol / std::pow(1.0_rad, 2))},
         {"deq", equilibrium_dihedral / 1.0_deg}};
    roundJSON(j, 6);
}

int HarmonicDihedral::numindex() const
{
    return 4;
}

HarmonicDihedral::HarmonicDihedral(double force_constant, double equilibrium_dihedral,
                                   const std::vector<int>& indices)
    : BondData(indices)
    , half_force_constant(force_constant / 2.0)
    , equilibrium_dihedral(equilibrium_dihedral)
{
}

BondData::Variant HarmonicDihedral::type() const
{
    return BondData::Variant::HARMONIC_DIHEDRAL;
}

std::shared_ptr<BondData> HarmonicDihedral::clone() const
{
    return std::make_shared<HarmonicDihedral>(*this);
}
...
void HarmonicDihedral::setEnergyFunction(const ParticleVector& particles)
{
    ...
    forceFunc = [&](Geometry::DistanceFunction distance) -> std::vector<IndexAndForce> {
        ...
        // Calculation of the energy derivative with respect to the dihedral angle.
        const auto force_magnitude =
            -2.0 * half_force_constant * (dihedral_angle - equilibrium_dihedral);
...
StretchData::StretchData(const std::vector<int>& indices)
    : BondData(indices)
{
}

int StretchData::numindex() const
{
    return 2;
}

TorsionData::TorsionData(const std::vector<int>& indices)
    : BondData(indices)
{
}

int TorsionData::numindex() const
{
    return 3;
}
...
TEST_CASE("[Faunus] BondData")
{
    ...
    Geometry::DistanceFunction distance_3a = [](const Point&, const Point&) -> Point {
        return {0, 3, 0};
    };
    Geometry::DistanceFunction distance_5a = [](const Point&, const Point&) -> Point {
        return {0, 3, 4};
    };
    ...
    SUBCASE("HarmonicBond")
    {
        ...
        {
            CHECK_NOTHROW(
                (R"({"harmonic": {"index":[0,9], "k":0.5, "req":2.1}})"_json).get<BondDataPtr>());
            CHECK_THROWS((R"({"harmoNIC": {"index":[2,3], "k":0.5, "req":2.1}})"_json)
                             .get<BondDataPtr>()); // exact match required
            CHECK_THROWS((R"({"harmonic": {"index":[2], "k":0.5, "req":2.1}})"_json)
                             .get<BondDataPtr>()); // 2 atom indices
            CHECK_THROWS((R"({"harmonic": {"index":[2,3], "req":2.1}})"_json)
                             .get<BondDataPtr>()); // k missing
            CHECK_THROWS((R"({"harmonic": {"index":[2,3], "k":2.1}})"_json)
                             .get<BondDataPtr>()); // req missing
        }
    }
    ...
    SUBCASE("FENEBond")
    {
        ...
        {
            CHECK_NOTHROW(
                (R"({"fene": {"index":[0,9], "k":0.5, "rmax":2.1}})"_json).get<BondDataPtr>());
            CHECK_THROWS(
                (R"({"FENE": {"index":[0,9], "k":0.5, "rmax":2.1}})"_json).get<BondDataPtr>());
            CHECK_THROWS(
                (R"({"fene": {"index":[2,3,4], "k":1, "rmax":2.1}})"_json).get<BondDataPtr>());
    ...
    SUBCASE("FENEWCABond")
    {
        ...
        {
            json j =
                R"({"fene+wca": {"index":[1,2], "k":8, "rmax":6.0, "eps":3.5, "sigma":4.5}})"_json;
            bond_ptr = j;                      // json --> object
            json j_roundtrip = json(bond_ptr); // object --> json
            roundJSON(j_roundtrip, 3);         // round decimals before comparison
            CHECK_EQ(j_roundtrip, j);
        ...
        {
            CHECK_NOTHROW(
                (R"({"fene+wca": {"index":[0,9], "k":1, "rmax":2.1, "eps":2.48, "sigma":2}})"_json)
                    .get<BondDataPtr>());
            CHECK_THROWS(
                (R"({"fene+wca": {"index":[2,3,4], "k":1, "rmax":2.1, "eps":2.48, "sigma":2}})"_json)
                    .get<BondDataPtr>());
            CHECK_THROWS(
                (R"({"fene+wca": {"index":[2,3], "rmax":2.1, "eps":2.48, "sigma":2}})"_json)
                    .get<BondDataPtr>());
            CHECK_THROWS((R"({"fene+wca": {"index":[2,3], "k":1, "eps":2.48, "sigma":2}})"_json)
                             .get<BondDataPtr>());
            CHECK_THROWS((R"({"fene+wca": {"index":[2,3], "k":1, "rmax":2.1, "eps":2.48}})"_json)
                             .get<BondDataPtr>());
            CHECK_THROWS((R"({"fene+wca": {"index":[2,3], "k":1, "rmax":2.1, "sigma":2}})"_json)
                             .get<BondDataPtr>());
        }
    }
    ...
    SUBCASE("HarmonicTorsion")
    {
        ...
        {
            CHECK_NOTHROW((R"({"harmonic_torsion": {"index":[0,1,9], "k":0.5, "aeq":30.5}})"_json)
                              .get<BondDataPtr>());
            CHECK_THROWS((R"({"harmonic_torsion": {"index":[2], "k":0.5, "aeq":30.5}})"_json)
                             .get<BondDataPtr>());
            CHECK_THROWS(
                (R"({"harmonic_torsion": {"index":[0,1,2], "aeq":30.5}})"_json).get<BondDataPtr>());
            CHECK_THROWS(
                (R"({"harmonic_torsion": {"index":[0,1,3], "k":0.5}})"_json).get<BondDataPtr>());
        }
    }
    ...
    SUBCASE("GromosTorsion")
    {
        SUBCASE("GromosTorsion Energy")
        {
            ...
            bond.setEnergyFunction(p_60deg_4a);
            CHECK_EQ(bond.energyFunc(distance),
                     Approx(100.0 / 2 * std::pow(cos(60.0_deg) - cos(45.0_deg), 2)));
        }
        ...
        {
            CHECK_NOTHROW((R"({"gromos_torsion": {"index":[0,1,9], "k":0.5, "aeq":30.5}})"_json)
                              .get<BondDataPtr>());
            CHECK_THROWS((R"({"gromos_torsion": {"index":[2], "k":0.5, "aeq":30.5}})"_json)
                             .get<BondDataPtr>());
            CHECK_THROWS(
                (R"({"gromos_torsion": {"index":[0,1,2], "aeq":30.5}})"_json).get<BondDataPtr>());
            CHECK_THROWS(
                (R"({"gromos_torsion": {"index":[0,1,3], "k":0.5}})"_json).get<BondDataPtr>());
        }
    }
    ...
    SUBCASE("PeriodicDihedral")
    {
        ...
        {
            json j =
                R"({"periodic_dihedral": {"index":[0,1,2,3], "k":10, "phi":0.0, "n": 3}})"_json;
            bond_ptr = j;
            CHECK_EQ(json(bond_ptr), j);
        ...
        {
            CHECK_NOTHROW(
                (R"({"periodic_dihedral": {"index":[0,1,2,9], "k":0.5, "phi":2.1, "n": 2}})"_json)
                    .get<BondDataPtr>());
            CHECK_THROWS(
                (R"({"periodic_dihedral": {"index":[0,1,2], "k":0.5, "phi":2.1, "n": 2}})"_json)
                    .get<BondDataPtr>());
            CHECK_THROWS((R"({"periodic_dihedral": {"index":[0,1,2,3], "phi":2.1, "n": 2}})"_json)
                             .get<BondDataPtr>());
            CHECK_THROWS((R"({"periodic_dihedral": {"index":[0,1,2,3], "k":0.5, "n": 2}})"_json)
                             .get<BondDataPtr>());
            CHECK_THROWS((R"({"periodic_dihedral": {"index":[0,1,2,3], "k":0.5, "phi":2.1}})"_json)
                             .get<BondDataPtr>());
        }
    }
    ...
    SUBCASE("HarmonicDihedral")
    {
        ...
        SUBCASE("HarmonicDihedral JSON")
        {
            ...
            CHECK_EQ(bond_ptr->energyFunc(distance),
                     Approx(100.0_kJmol / 2.0 * std::pow(30.0_deg, 2)));
        }
        SUBCASE("HarmonicDihedral JSON Invalid")
        {
            CHECK_NOTHROW((R"({"harmonic_dihedral": {"index":[0,1,2,3], "k":0.5, "deq":90}})"_json)
                              .get<BondDataPtr>());
            CHECK_THROWS((R"({"harmonic_dihedral": {"index":[0,1,2], "k":0.5, "deq":90}})"_json)
                             .get<BondDataPtr>());
            CHECK_THROWS((R"({"harmonic_dihedral": {"index":[0,1,2,3], "deq":90}})"_json)
                             .get<BondDataPtr>());
            CHECK_THROWS(
                (R"({"harmonic_dihedral": {"index":[0,1,2,3], "k":0.5}})"_json).get<BondDataPtr>());
        }
    }
    ...
    SUBCASE("Find")
    {
        ...
        CHECK_EQ(harmonic_bonds.front(),
                 bonds.back()); // harmonic_bonds should contain references to bonds
    }
}
