...
void ChainRotationMoveBase::_from_json(const json& j)
{
    ...
    dprot = j.at("dprot");
    allow_small_box =
        j.value("skiplarge", true); // todo rename the json attribute and make false default
}
...
void ChainRotationMoveBase::_to_json(json& j) const
{
    ...
    if (small_box_encountered > 0) {
        j["skipped"] = double(small_box_encountered) /
                       double(number_of_attempted_moves); // todo rename the json attribute
    }
    roundJSON(j, 3);
}
...
void ChainRotationMoveBase::_accept(Change&)
{
    msqdispl += sqdispl;
}

void ChainRotationMoveBase::_reject(Change&)
{
    msqdispl += 0;
}

double ChainRotationMoveBase::bias(Change&, double, double)
{
    return permit_move ? 0 : pc::infty;
}

ChainRotationMoveBase::ChainRotationMoveBase(Space& spc, const std::string& name,
...
void ChainRotationMove::rotate_segment(double angle)
{
    if (!segment_ndx.empty()) {
        ...
        // Uses an implementation from the old Pivot class. The translation of the chain might be
        // unnecessary.
        ...
        auto origin_pos =
            spc.particles.at(axis_ndx[0]).pos; // != shift_pos because of chain.translate
        auto axis_pos =
            spc.geometry.vdist(origin_pos, spc.particles.at(axis_ndx[1]).pos).normalized();
    ...
    if (!segment_ndx.empty()) {
        ...
        for (auto i : segment_ndx) {
            change_data.relative_atom_indices.push_back(
                i - offset); // `atoms` index are relative to chain
        }
        change_data.group_index =
            Faunus::distance(spc.groups.begin(), &chain); // integer *index* of moved group
...
{
    auto& chain = *molecule_iter;
    auto cm_pbc = Geometry::massCenter(chain.begin(), chain.end(), spc.geometry.getBoundaryFunc(),
                                       -chain.mass_center);
...
void CrankshaftMove::_from_json(const json& j)
{
    ...
    if (this->repeat < 0) {
        // set the number of repetitions to the length of the chain (minus 2) times the number of
        // the chains
...
size_t CrankshaftMove::select_segment()
{
    ...
    this->molecule_iter = this->spc.randomMolecule(
        this->molid, Faunus::move::CrankshaftMove::slump); // a random chain
    if (this->molecule_iter != this->spc.groups.end()) {
        ...
        if (molecule.size() > 2) { // must have at least three atoms
            auto joint0 =
                Faunus::move::CrankshaftMove::slump.sample(molecule.begin(), molecule.end());
            assert(joint0 >= molecule.begin() && joint0 < molecule.end());
            size_t range_begin =
                std::min(static_cast<size_t>(std::distance(molecule.begin(), joint0)), joint_max);
            size_t range_end =
                std::min(static_cast<size_t>(std::distance(joint0, molecule.end()) - 1), joint_max);
            auto joint1 =
                joint0 + Faunus::move::CrankshaftMove::slump.range(-range_begin, range_end);
...
size_t PivotMove::select_segment()
{
    ...
    if (this->molecule_iter != this->spc.groups.end()) {
        ...
        if (chain.size() > 2) { // must have at least three atoms
            auto bond = Faunus::move::PivotMove::slump.sample(
                bonds.begin(), bonds.end()); // a random harmonic bond
            if (bond != bonds.end()) {
