...
ClusterShapeAnalysis::ClusterShapeAnalysis(bool shape_anisotropy_use_com,
                                           const std::string& filename, bool dump_pqr_files)
    : save_pqr_files(dump_pqr_files)
    , shape_anisotropy_use_com(shape_anisotropy_use_com)
{
    if (!filename.empty()) {
...
ClusterShapeAnalysis::ClusterShapeAnalysis(const json& j)
    : ClusterShapeAnalysis(j.value("com", true), j.value("file", std::string()),
                           j.value("save_pqr", false))
{
}

decltype(ClusterShapeAnalysis::pqr_distribution)::iterator
ClusterShapeAnalysis::findPQRstream(size_t cluster_size)
{
...
double FindCluster::clusterProbability(const Group& group1, const Group& group2) const
{
    ...
    if (use_mass_center_threshold) {
        const auto mass_center_sepatation_squared =
            spc.geometry.sqdist(group1.mass_center, group2.mass_center);
        return static_cast<double>(mass_center_sepatation_squared <=
                                   thresholds_squared(group1.id, group2.id));
    }

    for (auto& particle_1 : group1) {
        for (auto& particle_2 : group2) {
            if (spc.geometry.sqdist(particle_1.pos, particle_2.pos) <=
                thresholds_squared(group1.id, group2.id)) {
                return 1.0;
            }
        }
...
void FindCluster::registerSatellites(const std::vector<std::string>& satellite_names)
{
    ...
    auto invalid_satellite = [&](auto molid) {
        return std::find(molids.begin(), molids.end(), molid) == molids.end();
    };
...
void FindCluster::parseThresholds(const json& j)
{
    if (j.is_number()) { // threshold is given as a single number
        ...
        if (j.size() != threshold_combinations) {
            throw ConfigurationError("exactly {} molecule pairs must be given in threshold matrix "
        ...
        for (const auto& [key, value] : j.items()) {
            if (auto name_pair = Faunus::splitConvert<std::string>(key); name_pair.size() == 2) {
                ...
                thresholds_squared.set(molecule1.id(), molecule2.id(),
                                       std::pow(value.get<double>(), 2));
            }
            else {
                throw ConfigurationError(
                    "threshold requires exactly two space-separated molecules");
            }
        }
    }
    else {
...
std::pair<std::vector<size_t>, bool> FindCluster::findCluster(size_t seed_index)
{
    ...
    for (auto it1 = cluster.begin(); it1 != cluster.end(); it1++) {
        for (auto it2 = pool.begin(); it2 != pool.end();) {
            const auto p = clusterProbability(spc.groups.at(*it1),
                                              spc.groups.at(*it2)); // probability to cluster
    ...
    auto mass_centers = cluster | rv::transform([&](auto i) -> const Point& {
                            return spc.groups.at(i).mass_center;
                        });
    ...
    assert(std::adjacent_find(cluster.begin(), cluster.end()) ==
           cluster.end()); // check for duplicates
...
{
    translate = std::make_unique<GroupTranslator>(j.at("dp").get<double>(),
                                                  j.value("dir", Point(1.0, 1.0, 1.0)));
...
std::function<void(Group&)> GroupTranslator::getLambda(Geometry::BoundaryFunction boundary,
                                                       Random& slump)
{
...
GroupRotator::GroupRotator(double displacement_factor, const Point& rotation_axis)
    : GroupMover(displacement_factor, (rotation_axis.squaredNorm() > pc::epsilon_dbl)
...
std::function<void(Group&)> GroupRotator::getLambda(Geometry::BoundaryFunction boundary,
                                                    const Point& rotation_origin, Random& random)
{
    if (std::fabs(displacement_factor) <= pc::epsilon_dbl) { // if zero displacement...
        return [](auto&) {};                                 // ...then return NOP lambda
    }
