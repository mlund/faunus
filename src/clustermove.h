...
class ClusterShapeAnalysis
{
  private:
    ...
    bool shape_anisotropy_use_com =
        true; //!< true if kappa^2 should be based on mass centers instead of particles
    ...
    findPQRstream(size_t cluster_size); //!< Create or find PQR files stream
...
class FindCluster
{
  private:
    ...
    bool use_mass_center_threshold =
        true;                  //!< use distance threshold between mass-centers instead of particles
    ...
    std::set<int>
        satellites; //!< subset of molecule id's to cluster, but NOT act as nuclei (cluster centers)
...
class Cluster : public Move
{
  private:
    ...
    Point
    clusterMassCenter(const std::vector<size_t>& indices) const; //!< Calculates cluster mass center
