...
{
    freesasa_parameters_fwd(const freesasa_parameters& p)
        : freesasa_parameters(p) {};
};
#endif
...
EwaldData::EwaldData(const json& j)
{
    ...
    use_spherical_sum = j.value(
        "spherical_sum", true); // Using spherical summation of k-vectors in reciprocal space?
    ...
    const_inf = (surface_dielectric_constant < 1)
                    ? 0
                    : 1; // if unphysical (<1) use epsr infinity for surrounding medium
...
PolicyIonIon::PolicyIonIon()
{
    cite = "doi:10.1063/1.481216";
}

PolicyIonIonIPBC::PolicyIonIonIPBC()
{
    cite = "doi:10/css8";
}
...
void PolicyIonIon::updateBox(EwaldData& d, const Point& box) const
{
    ...
    int k_vector_size =
        (2 * n_cutoff_ceil + 1) * (2 * n_cutoff_ceil + 1) * (2 * n_cutoff_ceil + 1) - 1;
    if (k_vector_size == 0) {
        ...
        for (int nx = 0; nx <= n_cutoff_ceil; nx++) {
            ...
            for (int ny = -n_cutoff_ceil * start_value; ny <= n_cutoff_ceil; ny++) {
                ...
                for (int nz = -n_cutoff_ceil * start_value; nz <= n_cutoff_ceil; nz++) {
                    ...
                    double k2 =
                        kv.squaredNorm() + d.kappa_squared; // last term is only for Yukawa-Ewald
...
void PolicyIonIonEigen::updateComplex(EwaldData& data, const Space::GroupVector& groups) const
{
    ...
    data.Q_ion.imag() =
        kr.array().sin().colwise().sum(); // imaginary part of 'Q^q', see eq. 25 in ref.
}

void PolicyIonIon::updateComplex(EwaldData& d, const Change& change,
...
TEST_CASE("[Faunus] Ewald - IonIonPolicy")
{
    ...
    SUBCASE("PBC")
    {
        ...
        CHECK_EQ(ionion.selfEnergy(data, c, spc.groups),
                 Approx(-1.0092530088080642 * data.bjerrum_length));
        CHECK_EQ(ionion.surfaceEnergy(data, c, spc.groups),
                 Approx(0.0020943951023931952 * data.bjerrum_length));
    ...
    /*SUBCASE("IPBCEigen") {
        ...
        CHECK_EQ(ionion.surfaceEnergy(data, c, spc.groups), Approx(0.0020943951023931952 *
    data.lB)); CHECK_EQ(ionion.reciprocalEnergy(data), Approx(0.0865107467 * data.lB));
    }*/
}
        ...
        for (int nx = 0; nx <= ncc; nx++) {
            ...
            for (int ny = -ncc * start_value; ny <= ncc; ny++) {
                ...
                for (int nz = -ncc * start_value; nz <= ncc; nz++) {
                    ...
                    double k2 =
                        kv.squaredNorm() + data.kappa_squared; // last term is only for Yukawa-Ewald
...
void PolicyIonIonIPBC::updateComplex(EwaldData& d, const Space::GroupVector& groups) const
{
    ...
    for (int k = 0; k < d.k_vectors.cols(); k++) {
        ...
        for (auto& g : groups) {
            for (auto& particle : g) {
                Q += q.cwiseProduct(particle.pos).array().cos().prod() *
...
void PolicyIonIonIPBC::updateComplex(EwaldData& d, const Change& change,
                                     const Space::GroupVector& groups,
                                     const Space::GroupVector& oldgroups) const
{
    assert(d.policy == EwaldData::IPBC or d.policy == EwaldData::IPBCEigen);
...
double PolicyIonIon::selfEnergy(const EwaldData& d, Change& change, Space::GroupVector& groups)
{
    ...
    double Vcc = -pc::pi / 2.0 / d.alpha / d.alpha /
                 (d.box_length[0] * d.box_length[1] * d.box_length[2]) * charge_total *
                 charge_total; // compensate with neutralizing background (if non-zero total charge
                               // in system)
...
void Ewald::force(std::vector<Point>& forces)
{
    ...
    for (auto& particle : spc.particles) {
        auto mu = particle.hasExtension() ? particle.getExt().mu * particle.getExt().mulen
                                          : Point(0, 0, 0);
    ...
    for (auto& particle : spc.particles) { // loop over particles
        (*force) =
            total_dipole_moment * particle.charge / (2.0 * data.surface_dielectric_constant + 1.0);
 ...
 */
void Ewald::setOldGroups(const Space::GroupVector& old_groups)
{
    this->old_groups = &old_groups;
}
...
void Ewald::to_json(json& j) const
{
    j = data;
}
...
TEST_CASE("[Faunus] Energy::Ewald")
{
    ...
    const auto reference_energy =
        (0.0020943951023931952 + 0.21303063979675319) * data.bjerrum_length;
    const auto energy_change = -16.8380445846; // move first position (0,0,0) --> (0.1, 0.1, 0.1)

    SUBCASE("energy")
    {
        CHECK_EQ(ewald.energy(change), doctest::Approx(reference_energy));
    }
    ...
    SUBCASE("update and restore (full update)")
    {
        ...
        space.updateParticles(positions.begin(), positions.end(), space.particles.begin(),
                              copy_position);
        CHECK((ewald.energy(change) !=
               doctest::Approx(reference_energy))); // no match since k-vectors not updated
        ewald.updateState(change);
    ...
    trial_space.updateParticles(positions.begin(), positions.end(), trial_space.particles.begin(),
                                copy_position);
    auto trial_ewald = Ewald(trial_space, data);
    CHECK_EQ(trial_ewald.energy(change), doctest::Approx(reference_energy));

    auto check_energy_change =
        [&](const Change& change) { // perturb from (0,0,0) --> (0.1, 0.1, 0.1)
            positions[0] = {0.0, 0.0, 0.0};
            trial_space.updateParticles(positions.begin(), positions.end(),
                                        trial_space.particles.begin(), copy_position);
...
{
    double s = 1 + std::sin(2.0 * pc::pi * particle.x()) +
               std::cos(2.0 * pc::pi * particle.y()) * static_cast<double>(use_2d);
    s *= scale_energy;
...
Example2D::Example2D(const json& j, Space& spc)
    : particle(spc.particles.at(0).pos)
{
...
{
    if (change &&
        spc.geometry.type != Geometry::Variant::CUBOID) { // no need to check in PBC systems
        // *all* groups
...
bool ContainerOverlap::groupIsOutsideContainer(const Change::GroupChange& group_change) const
{
    ...
    // *all* atoms
    if (group_change.all) {
        return std::any_of(group.begin(), group.end(), [&](auto const& particle) {
...
ContainerOverlap::ContainerOverlap(const Space& spc)
    : spc(spc)
{
...
Isobaric::Isobaric(const json& j, const Space& spc)
    : spc(spc)
{
    name = "isobaric";
...
const std::map<std::string, double> Isobaric::pressure_units = {
    {"P/atm", 1.0_atm},
    {"P/bar", 1.0_bar},
    {"P/kT", 1.0_kT},
    {"P/mM", 1.0_millimolar},
...
double Constrain::energy(const Change& change)
{
    if (change) {
        ...
        if (harmonic) {
            return harmonic->half_force_constant *
                   std::pow(harmonic->equilibrium_distance - value, 2);
        }
...
void Bonded::updateInternalBonds()
{
    ...
    std::for_each(spc.groups.begin(), spc.groups.end(),
                  [&](auto& group) { updateGroupBonds(group); });
}
...
double Bonded::sumBondEnergy(const Bonded::BondVector& bonds) const
{
    ...
    auto bond_energy = [&](const auto& bond) {
        return bond->energyFunc(spc.geometry.getDistanceFunc());
    };
...
Bonded::Bonded(const Space& spc, BondVector external_bonds = BondVector())
    : spc(spc)
    , external_bonds(std::move(external_bonds))
{
    name = "bonded";
    updateInternalBonds();
...
Bonded::Bonded(const json& j, const Space& spc)
    : Bonded(spc, j.value("bondlist", BondVector()))
{
}
    ...
    if (!internal_bonds.empty()) {
        ...
        for ([[maybe_unused]] const auto& [group_index, bonds] : internal_bonds) {
            std::for_each(bonds.begin(), bonds.end(),
                          [&](auto& bond) { array_of_bonds.push_back(bond); });
        }
    }
}
...
{
    std::for_each(energy_terms.cbegin(), energy_terms.cend(),
                  [&](auto energy) { j.push_back(*energy); });
}
...
{
    std::for_each(energy_terms.begin(), energy_terms.end(),
                  [&](auto energy) { energy->force(forces); });
}
    ...
    : energy_terms(this->vec)
{
    ...
    for (const auto& j_energy : j) { // loop over energy list
        try {
            ...
            try {
                if (key == "maxenergy") {
                    // looks like an unfortumate json scheme decision that requires special handling
                    // here
...
{
    std::for_each(energy_terms.begin(), energy_terms.end(),
                  [&](auto& energy) { energy->updateState(change); });
}
 ...
 */
std::unique_ptr<EnergyTerm> Hamiltonian::createEnergy(Space& spc, const std::string& name,
                                                      const json& j)
{
    ...
    try {
        if (name == "nonbonded_coulomblj" || name == "nonbonded_newcoulomblj") {
            return std::make_unique<Nonbonded<PairEnergy<CoulombLJ, false>, PairingPolicy>>(j, spc,
                                                                                            *this);
        }
        if (name == "nonbonded_coulomblj_EM") {
            return std::make_unique<NonbondedCached<PairEnergy<CoulombLJ, false>, PairingPolicy>>(
                j, spc, *this);
        }
        if (name == "nonbonded_splined") {
            return std::make_unique<
                Nonbonded<PairEnergy<pairpotential::SplinedPotential, false>, PairingPolicy>>(
                j, spc, *this);
        }
        if (name == "nonbonded" || name == "nonbonded_exact") {
            return std::make_unique<
                Nonbonded<PairEnergy<pairpotential::FunctorPotential, true>, PairingPolicy>>(j, spc,
                                                                                             *this);
        }
        if (name == "nonbonded_cached") {
            return std::make_unique<
                NonbondedCached<PairEnergy<pairpotential::SplinedPotential>, PairingPolicy>>(j, spc,
                                                                                             *this);
        }
        if (name == "nonbonded_coulombwca") {
            return std::make_unique<Nonbonded<PairEnergy<CoulombWCA, false>, PairingPolicy>>(j, spc,
                                                                                             *this);
        }
        if (name == "nonbonded_pm" || name == "nonbonded_coulombhs") {
            return std::make_unique<Nonbonded<PairEnergy<PrimitiveModel, false>, PairingPolicy>>(
                j, spc, *this);
        }
        if (name == "nonbonded_pmwca") {
            return std::make_unique<Nonbonded<PairEnergy<PrimitiveModelWCA, false>, PairingPolicy>>(
                j, spc, *this);
        }
        if (name == "bonded") {
...
const std::vector<double>& Hamiltonian::latestEnergies() const
{
    return latest_energies;
}

#ifdef ENABLE_FREESASA

FreeSASAEnergy::FreeSASAEnergy(const Space& spc, const double cosolute_molarity,
...
FreeSASAEnergy::FreeSASAEnergy(const json& j, const Space& spc)
    : FreeSASAEnergy(spc, j.value("molarity", 0.0) * 1.0_molar,
                     j.value("radius", 1.4) * 1.0_angstrom)
{
}
...
void FreeSASAEnergy::updateSASA(const Change& change)
{
    ...
    auto* result = freesasa_calc_coord(positions.data(), radii.data(), number_of_active_particles,
                                       parameters.get());
...
void FreeSASAEnergy::to_json(json& j) const
{
    ...
    j[unicode::bracket("SASA") + "/" + unicode::angstrom + unicode::squared] =
        mean_surface_area.avg() / 1.0_angstrom;
...
TEST_CASE("[Faunus] FreeSASA")
{
    ...
    SUBCASE("Separated atoms")
    {
        ...
        CHECK_EQ(sasa.energy(change),
                 Approx(4.0 * pc::pi * (3.4 * 3.4 + 2.6 * 2.6) * 1.5 * 1.0_kJmol));
    }
 ...
 * @param dense_container flag specifying if a fast memory heavy version of cell_list container is
 * used
 */
SASAEnergyReference::SASAEnergyReference(const Space& spc, double cosolute_molarity,
                                         double probe_radius, int slices_per_atom,
    ...
    , cosolute_molarity(cosolute_molarity)
{
    ...
    case 3: // PBC in all directions
        if (dense_container) {
            sasa = std::make_unique<SASACellList<SASA::DensePeriodicCellList>>(spc, probe_radius,
                                                                               slices_per_atom);
        }
        else {
            sasa = std::make_unique<SASACellList<SASA::SparsePeriodicCellList>>(spc, probe_radius,
                                                                                slices_per_atom);
        }
        break;
    case 0:
        if (dense_container) {
            sasa = std::make_unique<SASACellList<SASA::DenseFixedCellList>>(spc, probe_radius,
                                                                            slices_per_atom);
        }
        else {
            sasa = std::make_unique<SASACellList<SASA::SparseFixedCellList>>(spc, probe_radius,
                                                                             slices_per_atom);
        }
        break;
    default:
...
SASAEnergyReference::SASAEnergyReference(const json& j, const Space& spc)
    : SASAEnergyReference(spc, j.at("molarity").get<double>() * 1.0_molar,
                          j.value("radius", 1.4) * 1.0_angstrom, j.value("slices", 25),
                          j.value("dense", true))
{
}
 ...
 * used
 */
SASAEnergy::SASAEnergy(const Space& spc, double cosolute_molarity, double probe_radius,
...
SASAEnergy::SASAEnergy(const json& j, const Space& spc)
    : SASAEnergy(spc, j.at("molarity").get<double>() * 1.0_molar,
                 j.value("radius", 1.4) * 1.0_angstrom, j.value("slices", 25),
                 j.value("dense", true))
{
}
...
const std::vector<double>& SASAEnergyReference::getAreas() const
{
    return areas;
}
...
{
    //!< if the state is ACCEPTED there is no need to recalculate SASAs so return empty
    //!< target_indices
    if (state == MonteCarloState::ACCEPTED) {
        return;
    }
...
**/
void SASAEnergy::insertChangedNeighboursOf(const index_type index,
                                           std::set<index_type>& target_indices) const
{
...
void SASAEnergy::sync(EnergyTerm* energybase_ptr, const Change& change)
{
    if (auto* other = dynamic_cast<SASAEnergy*>(energybase_ptr)) {
        const auto sync_data = [this, other](const size_t changed_index) {
            this->current_neighbours.at(changed_index) =
                other->current_neighbours.at(changed_index);
...
TEST_CASE_TEMPLATE("[Faunus] SASAEnergy_updates", EnergyTemplate, SASAEnergyReference, SASAEnergy)
{
    ...
    SUBCASE("Update everything")
    {
        ...
        for (const auto& [area, ref_area] :
             ranges::views::zip(sasa_energy.getAreas(), ref_energy.getAreas())) {
    ...
    SUBCASE("Partial update")
    {
        ...
        // these are usually obtained from previous MC step in simulations so here we need to
...
GroupCutoff::GroupCutoff(Space::GeometryType& geometry)
    : geometry(geometry)
{
}
            ...
            for (const auto& [named_pair, pair_cutoff] : it->items()) {
                ...
                try {
                    if (const auto molecules_names = splitConvert<std::string>(named_pair);
                        ...
                        faunus_logger->debug("custom cutoff for {}-{} = {} Å", molecule1.name,
                                             molecule2.name, pair_cutoff.get<double>());
                    }
                    else {
                    ...
                    throw ConfigurationError("Unable to set a custom cutoff for {}: {}", named_pair,
                                             e.what());
                }
            }
        }
...
EnergyAccumulatorBase::EnergyAccumulatorBase(double value)
    : value(value)
{
}
...
void EnergyAccumulatorBase::clear()
{
    value = 0.0;
}

EnergyAccumulatorBase::operator double()
{
    return value;
}
...
void EnergyAccumulatorBase::to_json(json& j) const
{
    j["summation_policy"] = scheme;
}
...
double CustomGroupGroup::energy([[maybe_unused]] const Change& change)
{
    ...
    auto group_group_energy = [&](auto index1, auto index2) -> double {
        ...
        if (((group1.id == molid1) & (group2.id == molid2)) |
            ((group1.id == molid2) & (group2.id == molid1))) {
...
void CustomGroupGroup::setParameters(const Group& group1, const Group& group2)
{
    ...
    properties.mass_center_separation =
        sqrt(spc.geometry.sqdist(group1.mass_center, group2.mass_center));
}

void CustomGroupGroup::to_json(json& j) const
{
    j = json_input_backup;
    j["mean_charges"] = {{"Z1", mean_charges.at(molid1).avg()},
                         {"Z2", mean_charges.at(molid2).avg()}};
};
