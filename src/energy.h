 ...
 * MC move acceptance.
 */
template <typename Tspace> class Penalty : public Energybase {
  protected:
    typedef typename Tspace::Tparticle Tparticle;
    typedef typename Tspace::Tgroup Tgroup;
    typedef typename Tspace::Tpvec Tpvec;
                ...
                if (i.size() == 1) {
                    ...
                    for (auto it = i.begin(); it != i.end(); ++it) {
                        ...
                        try {
                            ...
                            throw std::runtime_error("error for reaction coordinate '" + it.key() + "': " + e.what() +
...
#ifdef ENABLE_MPI
template <typename Tspace, typename Base = Penalty<Tspace>> struct PenaltyMPI : public Base {
    using Base::cnt;
    using Base::coord;
    using Base::f0;
    using Base::file;
...
template <typename Tspace> class Hamiltonian : public Energybase, public BasePointerVector<Energybase> {
  protected:
            ...
            for (auto it = m.begin(); it != m.end(); ++it) {
                try {
                    ...
                    if (it.key() == "nonbonded_exact")
                        push_back<Energy::Nonbonded<Tspace, FunctorPotential<typename Tspace::Tparticle>>>(it.value(),
                                                                                                           spc);
