...
/** @section Force moves
 *
 * Force moves are pseudo-Monte-Carlo moves that run a short trajectory of Langevin or similar
 * dynamics. The move is always accepted regardless of the total potential energy of the final
 * system's configuration.
 *
 * ForceMoveBase provides an interface for all dynamics moves. It orchestrates integrators,
 * thermostats, etc. to work in accord. IntegratorBase provides an interface for all integrators
 * that are responsible for development of the simulation in time steps. The integrators update
 * positions and velocities of particles as needed.
 */
...
class NormalRandomVector
{
  ...
  public:
    ...
    {
        return {normal_distribution(engine), normal_distribution(engine),
 ...
 *
 * Integrators progress the simulation in time steps. Positions and velocities of the particles as
 * well as forces acting on them are updated in every time step as prescribed by the given
 * integrator scheme.
 */
class IntegratorBase
{
  ...
  public:
    /**
     * @brief Move particles by one time step and update their positions, velocities and acting
     * forces
     * @param velocities  a vector of particles' velocities
     * @param forces  a vector of particles' forces
     */
    virtual void step(PointVector& velocities,
                      PointVector& forces) = 0; // todo shall we return real time progress?
...
class LangevinVelocityVerlet : public IntegratorBase
{
    ...
    NormalRandomVector
        random_vector; //!< generator of random 3d vectors from the normal distribution
    [[nodiscard]] inline Point positionIncrement(
        const Point& velocity) const; //!< increment particle's position in an A semi-step
    [[nodiscard]] inline Point
    velocityIncrement(const Point& force,
                      double mass) const; //!< increment particle's velocity in a B semi-step
    //! apply fluctuation-dissipation to particle's velocity by solving of Ornstein-Uhlenbeck
    //! process in an O-step
  ...
  public:
    ...
    LangevinVelocityVerlet(Space& spc, Energy::EnergyTerm& energy, double time_step,
 ...
 *
 * Orchestrate execution of integrators, thermostats, etc. Store vectors for velocities and forces,
 * which are not part of the particle vector in the space.
 */
class ForceMove : public Move
{
  protected:
    ...
    unsigned int
        number_of_steps;    //!< number of integration steps to perform during a single MC move
    ...
    size_t resizeForcesAndVelocities(); //!< Resize velocities and forces to match number of active
                                        //!< particles
    void generateVelocities();          //!< Generate initial velocities and zero forces
    void _to_json(json& j) const override;
    void _from_json(const json& j) override;
    void _move(Change& change) override;
    ForceMove(Space& spc, const std::string& name, const std::string& cite,
