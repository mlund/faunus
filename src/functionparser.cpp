...
template <std::floating_point T> void ExprFunction<T>::set(const nlohmann::json& j, const Tvarvec& vars)
{
    Tconstvec consts;
...
template <std::floating_point T> T ExprFunction<T>::operator()() const { return expression->value(); }
