...
namespace Faunus {
  ...
  */
  ...
  //--------------- IOAAM ---------------------
  ioaam::ioaam(atoms &spc) : iopart(spc) {}
  string ioaam::p2s(particle &p, int i) {
    std::ostringstream o;
    o.precision(5);
    o << (*atomPtr)[p.id].name << " " << i+1 << " "
      << p.x<<" "<< p.y<<" "<<p.z<<" "
  ...
  particle ioaam::s2p(string &s) {
    ...
    p.id = static_cast<particle::type>((*atomPtr)[name].id);
    p.hydrophobic = (*atomPtr)[p.id].hydrophobic;
    return p;
  }
  ...
  //----------------- IOXYZ ----------------------
  //ioxyz::ioxyz(atoms &spc) : iopart(spc) {
  //}
  ioxyz::ioxyz(atoms &s) : iopart(s) {
  }
   ...
   */
  iogro::iogro(atoms &s, inputfile &in) : iopart(s) {
  ...
  //----------------- IOPOV ----------------------
  //iopov::iopov(atoms &spc) : iopart(spc) {
  iopov::iopov(container &c) : iopart(c.atom) {
    clear();
    header();
    light(100.);
    camera();
