/*
 * $Id: /local/xtcio/src/xdrf.c 39 2008-09-14T06:31:00.970588Z lidb  $
 * 
...
int 
_xtc_get_next_frame_number(int fp,int natoms)
{
    gmx_off_t off;
    gmx_off_t next_off;
    int inp;  
    ...
    while(1) {
         ...
         && xdr_int(xdridptr[fp+1], &inp)) 
      {
        ...
        if(gmx_fseek(xdrfiles[fp+1], next_off, SEEK_SET)){
          return -1;
        }
      }
    }
...
float 
_xtc_get_next_frame_time(int fp,int natoms, bool * bOK)
{
    gmx_off_t off;
    gmx_off_t next_off;
    int inp;  
...
float 
_xtc_get_current_frame_time(int fp,int natoms, bool * bOK)
{
    gmx_off_t off;
    gmx_off_t next_off;
    int inp;  
...
int 
_xtc_get_current_frame_number(int fp,int natoms, bool * bOK)
{
    gmx_off_t off;
    gmx_off_t next_off;
    int inp;  
...
gmx_off_t 
_xtc_get_next_frame_start(int fp, int natoms)
{
    int inp;
    int res;
    ...
    while(xdr_int(xdridptr[fp+1],&inp))
    {
	  ...
	  if(xdr_int(xdridptr[fp+1],&inp) && inp == natoms)
		...
		return res;
              ...
              gmx_fseek(xdrfiles[fp+1], -4, SEEK_CUR);
            }
        }
    }
    return -1;
...
float 
_xtc_estimate_dt(int fp, int natoms, bool * bOK)
{
  float  res;
  float  tinit;
  gmx_off_t off;
  ...
  if((off   = gmx_ftell(xdrfiles[fp+1])) < 0){
    ...
    tinit = _xtc_get_current_frame_time(fp,natoms,bOK);
    ...
    *bOK = 1;
    ...
    res = _xtc_get_next_frame_time(fp,natoms,bOK);
