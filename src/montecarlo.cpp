...
bool MetropolisMonteCarlo::metropolisCriterion(const double energy_change)
{
    ...
    const auto random_number_between_zero_and_one =
        move::Move::slump(); // engine *must* be propagated!
    if (std::isinf(energy_change) &&
        energy_change < 0.0) { // if negative infinity -> quietly accept
        return true;
    }
...
void MetropolisMonteCarlo::init()
{
    ...
    state->pot->state =
        Energy::EnergyTerm::MonteCarloState::ACCEPTED; // this is the old energy (current, accepted)
    trial_state->pot->state =
        Energy::EnergyTerm::MonteCarloState::TRIAL; // this is the new energy (trial)
    ...
    : original_log_level(faunus_logger->level())
{
    ...
    moves = std::make_unique<move::MoveCollection>(j.at("moves"), *trial_state->spc,
                                                   *trial_state->pot, *state->spc);
    init();
}
...
void MetropolisMonteCarlo::performMove(move::Move& move)
{
    ...
    if (change) {
        ...
        const auto energy_bias =
            move.bias(change, old_energy, new_energy) +
        ...
        if (std::isfinite(initial_energy)) {
            average_energy +=
                initial_energy + sum_of_energy_changes; // update average potential energy
        }
    }
    else {
...
Space& MetropolisMonteCarlo::getSpace()
{
    return *state->spc;
}

Space& MetropolisMonteCarlo::getTrialSpace()
{
    return *trial_state->spc;
}
...
double TranslationalEntropy::atomSwapEnergy(const Change::GroupChange& group_change) const
{
    ...
    double energy = 0.0;
    int id1 = trial_spc.groups.at(group_change.group_index)
                  .at(group_change.relative_atom_indices.front())
                  .id;
    int id2 =
        spc.groups.at(group_change.group_index).at(group_change.relative_atom_indices.front()).id;
...
{
    auto mollist_new =
        trial_spc.findMolecules(molid, Space::Selection::ALL); // "ALL" because "ACTIVE"
    auto mollist_old =
        spc.findMolecules(molid, Space::Selection::ALL); // ...returns only full groups
...
double TranslationalEntropy::energy(const Change& change)
{
    ...
    std::set<MoleculeData::index_type>
        already_processed; // ignore future encounters of these molid's
    for (const Change::GroupChange& data : change.groups) { // loop over each change group
        if (data.dNswap) { // number of atoms has changed as a result of a swap move
            ...
            if (data.dNatomic and
                Faunus::molecules.at(molid).isAtomic()) { // an atomic group has been changed
                energy_change += atomChangeEnergy(molid);
            }
            else if (!already_processed.contains(
                         molid)) { // a molecule has been inserted or deleted
