        ...
        template<typename Tspace>
            class BiasedTranslateRotate : public Movebase {
                protected:
                    ...
                    double radius;
                    double _sqd; // squared displacement
                    Average<double> msqd, countNavg, countNavgBlocks, countNout_avg, countNout_avgBlocks; // mean squared displacement, water molecules inside, and outside sphere
                    double dist, distNew, distTemp;
                    double randNbr;
                    double _bias=0, sigma = 0, countN, countNout, cntInner=0, countNtot=0, std=1; 
                    Point dir={1,1,1};
                    Point origo={0,0,0};
                    
                    bool findBias = true;
                    
                    //AverageExt<double> n(100);
                    ...
                    void _to_json(json &j) const override {
                        j = {
                            ...
                            {u8::rootof + u8::bracket("r" + u8::squared), std::sqrt(msqd.avg())},
                            {"# of water in sphere in last loop", countN},
                            {"Average # of water inside sphere", countNavg.avg()},
                            {"Average # of water outside sphere", countNout_avg.avg()},
                            {"Standard deviation of water inside sphere", countNavg.stdev()},
                        ...
                        if (size(mollist)>0) {
                            ...
                            if (not it->empty()) {
                                ...
                                if ( not ( dist > radius && p < randNbr ) ) {
                                    ...
                                    if (cnt%10000==0){
                                        countNavgBlocks += countNavg.avg();
                                        countNout_avgBlocks += countNout_avg.avg();
                                    } 
                                    
                                    if (cnt%1000000 == 0) {              
                                        cout << "Average # of water molecules inside sphere: " << countNavgBlocks.avg() << "\n";
                                        cout << "Relative standard deviation based on molecules inside sphere: " << countNavgBlocks.stdev()/countNavgBlocks.avg() << "\n";
                                    ...
                                    */
                                    ...
                                    /*    
                                      ...
                                      */  
                                        ...
                                        _bias = -log(1/(1+(1-p)/(p*countNtot+(1-p)*countN)));
                                        /*
                                        cout << "Old distance: " << dist << ", new distance: " << distNew << "\n";
