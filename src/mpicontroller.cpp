...
{
    if (rank.has_value()) {
        // note `std::minmax(a,b)` takes _references_; the initializer list (used here) takes a
        // _copy_
...
ParticleBuffer::Format ParticleBuffer::getFormat() const
{
    return format;
}
...
{
    setFormat(Format::XYZQI);
}

ParticleBuffer::buffer_iterator ParticleBuffer::begin()
{
    return buffer.begin();
}

ParticleBuffer::buffer_iterator ParticleBuffer::end()
{
    return buffer.end();
}
                                                    ...
                                                    const ParticleVector& particles)
{
    ...
    mpi.world.sendrecv_replace(particle_buffer.begin(), particle_buffer.end(), partner_rank,
                               mpl::tag_t(0), partner_rank, mpl::tag_t(0));
 ...
 */
void ExchangeParticles::replace(const mpl::communicator& comm, int partner_rank,
                                ParticleVector& particles)
{
    particle_buffer.copyToBuffer(particles); // particle data -> vector of doubles
    comm.sendrecv_replace(particle_buffer.begin(), particle_buffer.end(), partner_rank,
                          mpl::tag_t(0), partner_rank, mpl::tag_t(0));
    particle_buffer.copyFromBuffer(particles);
}

ParticleBuffer::Format ExchangeParticles::getFormat() const
{
    return particle_buffer.getFormat();
}

void ExchangeParticles::setFormat(ParticleBuffer::Format format)
{
    particle_buffer.setFormat(format);
}
                    ...
                    Geometry::VolumeMethod& volume_scaling_method)
{
    ...
    auto new_volume = 0.0;
    mpi.world.sendrecv(old_volume, partner_rank, mpl::tag_t(0), new_volume, partner_rank,
                       mpl::tag_t(0));
