...
void Penalty::initializePenaltyFunction(const json& j)
{
    ...
    for (const auto& i : j) {
        auto& reaction_coordinate = reaction_coordinates_functions.emplace_back(
            ReactionCoordinate::createReactionCoordinate(i, spc));
        if (reaction_coordinate->minimum_value >= reaction_coordinate->maximum_value ||
            reaction_coordinate->resolution <= 0) {
            throw ConfigurationError(
                "min<max and resolution>0 required for penalty reaction coordinate");
        }
...
void Penalty::loadPenaltyFunction(const std::string& filename)
{
    if (std::ifstream stream(filename); stream) {
        ...
        std::string ignore;
        stream >> ignore >> energy_increment >> samplings >>
            penalty_function_exchange_counter; // header line
...
void Penalty::updatePenalty(const std::vector<double>& coordinate)
{
    ...
    if (update_counter % number_of_steps_between_updates == 0 and energy_increment > 0.0) {
        if (histogram.minCoeff() >= (int)samplings) {
            const auto minimum_penalty_energy =
                penalty_energy.minCoeff(); // define min. penalty energy...
...
{
    faunus_logger->info("energy barriers: penalty function = {} kT, histogram = {} kT",
                        penalty_energy.maxCoeff(),
...
void Penalty::sync(EnergyTerm* other, [[maybe_unused]] const Change& change)
{
    ...
    other_penalty->updatePenalty(
        other_penalty->latest_coordinate); // keep update_counter and samplings in sync
...
void PenaltyMPI::averagePenaltyFunctions()
{
    ...
    if (mpi.isMaster()) {
        ...
        for (int i = 0; i < mpi.world.size(); i++) {
            ...
            penalty_energy += Eigen::Map<Eigen::MatrixXd>(
                buffer.data() + offset, penalty_energy.rows(), penalty_energy.cols());
        }
        penalty_energy = (penalty_energy.array() - penalty_energy.minCoeff()) /
                         static_cast<double>(mpi.world.size());
    }
