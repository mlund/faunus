...
class Penalty : public EnergyTerm
{
  ...
  protected:
    ...
    std::vector<Tcoord> reaction_coordinates_functions; //!< vector of reaction coordinate functions
                                                        //!< (length = 1 or 2)
...
class PenaltyMPI : public Penalty
{
  private:
    ...
    void updatePenalty(const std::vector<double>& coordinate)
        override;                   //!< Average penalty function across all nodes
