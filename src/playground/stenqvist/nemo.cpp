...
class IonIonFanourgakis : public Coulomb {
  ...
  public:
    IonIonFanourgakis(InputMap &in, const string &dir="") : Coulomb(in) { 
      ...
      //cout << "_lB: IonIonFanourgakis: " << _lB << endl;
    }
...
void generateLattice(Tspace &spc, double RcC, double side, double charge, int &cnt, int &closeCenter, int case0) {
  ...
  if(case0 == 0) {
    ...
    for(double x = -RcC; x < RcC+side; x += side) {
      ...
      for(double y = -RcC; y < RcC+side; y += side) {
        for(double z = -RcC; z < RcC+side; z += side) {
          Point vec = Point(x/2.0,y+extra,z+extra);
          double r2 = vec.squaredNorm();
          if(r2 < RcC2) {
            if(r2 < 1e-6)
              closeCenter = cnt;
            spc.p[cnt] = Point(x/2.0,y+extra,z+extra);
            spc.p[cnt].mu = Point(charge,0,0);
            spc.p[cnt].muscalar = 1.0;
            spc.p[cnt++].charge = charge;
          }
...
int main() {
  ...
  int case0 = 2;    // 0 = SodiumChloride, 1 = Dipole, 2 = CesiumChloride
  double kappa = 0.8;
  int writePos = 1;
