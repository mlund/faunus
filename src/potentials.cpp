...
TCombinatorFunc PairMixer::getCombinator(CombinationRuleType combination_rule,
    ...
    for (const auto& i : atoms) {
        for (const auto& j : atoms) {
            if (i.implicit || j.implicit) {
                // implicit atoms are ignored as the missing properties, e.g., sigma and epsilon,
                // might raise errors
                (*matrix)(i.id(), j.id()) = combUndefined();
            }
            else if (i.id() == j.id()) {
                // if the combinator is "undefined" the homogeneous interaction is still
                // well-defined
                (*matrix)(i.id(), j.id()) = modifier(extractor(i.interaction));
            }
            else {
                (*matrix)(i.id(), j.id()) =
                    modifier(combinator(extractor(i.interaction), extractor(j.interaction)));
            }
        }
    }
    return matrix;
...
TEST_CASE("[Faunus] PairMixer")
{
    ...
    SUBCASE("Enumerated potential")
    {
        ...
        CHECK_EQ(PairMixer::getCombinator(CombinationRuleType::LORENTZ_BERTHELOT,
                                          PairMixer::CoefficientType::SIGMA)(2.0, 8.0),
                 PairMixer::combArithmetic(2.0, 8.0));
        CHECK_EQ(PairMixer::getCombinator(CombinationRuleType::LORENTZ_BERTHELOT,
                                          PairMixer::CoefficientType::EPSILON)(2.0, 8.0),
                 PairMixer::combGeometric(2.0, 8.0));
        CHECK_THROWS_AS(PairMixer::getCombinator(CombinationRuleType::LORENTZ_BERTHELOT),
                        std::logic_error);

        SUBCASE("")
        {
            atoms = R"([{"A": {"sigma":2.0}}, {"B": {"sigma":8.0}}, {"C": {"sigma":18.0}}])"_json
                        .get<decltype(atoms)>();
            REQUIRE((atoms.front().interaction.at("sigma") == Approx(2.0)));
            std::vector<CustomInteractionData> pairs =
            ...
            {
                CHECK_NOTHROW(
                    R"({"A C": {"sigma": 9.5}})"_json.get<std::vector<CustomInteractionData>>());
...
void from_json(const json& j, CustomInteractionData& c)
{
    ...
    try {
        ...
        if (atom_ids.size() != c.atom_id.size()) {
            faunus_logger->error(
                "Custom interaction parameters require exactly {} space-separated atoms: {}.",
        ...
        faunus_logger->error("Unknown atom pair [{}] in custom interaction parameters.",
                             j_item.key());
...
void MixerPairPotentialBase::init()
{
    ...
    faunus_logger->debug("Combination rule {} in effect for the {} potential.",
                         j_combination_rule.get<std::string>(), name);
    initPairMatrices();
}
    ...
    try {
        if (j.contains("mixing")) {
            ...
            if (combination_rule == CombinationRuleType::UNDEFINED && mixing != "undefined") {
                // an ugly hack because the first pair in the json ↔ enum mapping is silently
                // selected by default
                throw PairPotentialException("unknown combination rule " +
                                             mixing.get<std::string>());
            }
        }
        if (j.contains("custom")) {
            // *custom_pairs = j["custom"]; // does not work, perhaps as from_json is also a method
...
void RepulsionR3::to_json(json& j) const
{
...
void DipoleDipole::from_json(const json& j)
{
...
void FENE::to_json(json& j) const
{
...
TEST_CASE("[Faunus] SASApotential")
{
    ...
    CHECK_EQ(pot(a, b, 10 * 10, {10, 0, 0}),
             Approx(f * 4 * pc::pi * (2.1 * 2.1 + 1.5 * 1.5)));                 // far apart
    ...
    SUBCASE("energy")
    {
        ...
        pairpotential::from_json(R"({"constants": { "kappa": 30, "lB": 7},
                              "function": "lB * charge1 * charge2 / (s1+s2) * exp(-kappa/r) * kT + pi"})"_json,
                                 pot);
        CHECK_EQ(pot(a, b, 2 * 2, {0, 0, 2}),
                 Approx(-7.0 / (3.0 + 4.0) * std::exp(-30.0 / 2.0) * pc::kT() + pc::pi));
    }
    SUBCASE("force")
    {
        CustomPairPotential pot;
        ...
        pairpotential::from_json(R"({ "coulomb": {"epsr": 80.0, "type": "plain"} } )"_json,
                                 coulomb);
...
void LennardJones::initPairMatrices()
{
    ...
    faunus_logger->trace(
        "Pair matrices for {} sigma ({}×{}) and epsilon ({}×{}) created using {} custom pairs.",
...
LennardJones::LennardJones(const std::string& name, const std::string& cite,
...
TEST_CASE("[Faunus] LennardJones")
{
    ...
    {
        CHECK_THROWS_AS(makePairPotential<LennardJones>(R"({"mixing": "unknown"})"_json),
                        std::runtime_error);
        CHECK_NOTHROW(makePairPotential<LennardJones>(R"({})"_json));
        // alternative notation for custom as an array: custom: []
        CHECK_NOTHROW(makePairPotential<LennardJones>(
    ...
    {
        using doctest::Approx;
        auto lj = makePairPotential<LennardJones>(
            R"({"mixing": "LB", "custom": [{"A B": {"eps": 0.5, "sigma": 8}}]})"_json);
    ...
    SUBCASE("Force")
    {
        using doctest::Approx;
        auto lj = makePairPotential<LennardJones>(
            R"({"mixing": "LB", "custom": [{"A B": {"eps": 2.0, "sigma": 8}}]})"_json);
        a.pos = {0, 0, 0};
        b.pos = {9, 0, 0};
...
{
    sigma_squared =
        PairMixer(extract_sigma, PairMixer::getCombinator(combination_rule), &PairMixer::modSquared)
            .createPairMatrix(atoms, *custom_pairs);
    faunus_logger->trace("Pair matrix for {} sigma ({}×{}) created using {} custom pairs.", name,
...
TEST_CASE("[Faunus] HardSphere")
{
    ...
    SUBCASE("JSON initialization")
    {
        ...
        CHECK_THROWS_AS(makePairPotential<HardSphere>(R"({"mixing": "unknown"})"_json),
                        std::runtime_error);
    }
...
{
    const TCombinatorFunc comb_diameter =
        PairMixer::getCombinator(combination_rule, PairMixer::CoefficientType::SIGMA);
    const TCombinatorFunc comb_epsilon =
        PairMixer::getCombinator(combination_rule, PairMixer::CoefficientType::EPSILON);
    ...
    faunus_logger->trace(
        "Pair matrix for {} radius ({}×{}) and epsilon ({}×{}) created using {} custom pairs.",
        name, sigma_squared->rows(), sigma_squared->cols(), epsilon->rows(), epsilon->cols(),
        custom_pairs->size());
}
...
TEST_CASE("[Faunus] Hertz")
{
    ...
    {
        json hertz_json =
            R"({ "hertz": {"mixing": "lorentz_berthelot", "eps": "eps", "sigma": "sigma"}})"_json;
...
void SquareWell::initPairMatrices()
{
    const TCombinatorFunc comb_diameter =
        PairMixer::getCombinator(combination_rule, PairMixer::CoefficientType::SIGMA);
    const TCombinatorFunc comb_depth =
        PairMixer::getCombinator(combination_rule, PairMixer::CoefficientType::EPSILON);
...
TEST_CASE("[Faunus] SquareWell")
{
    ...
    SUBCASE("JSON initilization")
    {
        CHECK_THROWS_AS(makePairPotential<SquareWell>(R"({"mixing": "unknown"})"_json),
                        std::runtime_error);
...
void Polarizability::from_json(const json& j)
{
    ...
    for (const auto& i : Faunus::atoms) {
        for (const auto& j : Faunus::atoms) {
            m_neutral->set(i.id(), j.id(),
                           -3 * i.alphax * pow(0.5 * i.sigma, 3) * j.alphax *
                               pow(0.5 * j.sigma, 3));
...
void Polarizability::to_json(json& j) const
{
    j = {{"epsr", epsr}};
}
        ...
        for (auto& [name, j_config] : single_record.items()) {
            ...
            try {
                ...
                // terms if not already added
                    ...
                    faunus_logger->error(
                        "'{}' is deprecated, use 'lennardjones'+'multipole' instead", name);
                }
...
void FunctorPotential::to_json(json& j) const
{
    ...
    j["selfenergy"] = {{"monopole", have_monopole_self_energy},
                       {"dipole", have_dipole_self_energy}};
}
...
void FunctorPotential::from_json(const json& j)
{
    ...
    umatrix =
        decltype(umatrix)(atoms.size(), combinePairPotentials(backed_up_json_input.at("default")));
...
TEST_CASE("[Faunus] FunctorPotential")
{
    ...
    auto coulomb =
        pairpotential::makePairPotential<Coulomb>(R"({ "coulomb": {"epsr": 80.0} } )"_json);
    auto wca = pairpotential::makePairPotential<WeeksChandlerAndersen>(
        R"({ "wca" : {"mixing": "LB"} })"_json);
...
SplinedPotential::KnotData::KnotData(const base& b)
    : base(b)
{
}
    ...
    while (rmin >= dr) {
        if (num_iterations++ == max_iterations) {
            throw std::runtime_error(
                "Pair potential spline error: cannot determine minimum distance");
        }
        double u = std::fabs(
            FunctorPotential::operator()(particle1, particle2, rmin * rmin, {rmin, 0, 0}));
...
double SplinedPotential::findUpperDistance(int i, int j, double energy_threshold, double rmax)
{
    ...
    while (rmax >= dr) {
        if (num_iterations++ == max_iterations) {
            throw std::runtime_error(
                "Pair potential spline error: cannot determine maximum distance");
        }
...
SplinedPotential::SplinedPotential(const std::string& name)
    : FunctorPotential(name)
{
}
...
void SplinedPotential::createKnots(int i, int j, double rmin, double rmax)
{
    ...
    if (knotdata.hardsphere_repulsion) {
        faunus_logger->trace("Hardsphere repulsion enabled for {}-{} spline",
                             Faunus::atoms.at(i).name, Faunus::atoms.at(j).name);
    }
    ...
    for (const auto r : arange(rmin + dr, rmax, dr)) {
        auto error =
            std::fabs(operator()(particle1, particle2, r * r, {r, 0, 0}) -
                      FunctorPotential::operator()(particle1, particle2, r * r, {r, 0, 0}));
        max_error = std::max(error, max_error);
    }
    faunus_logger->trace("{}-{} interaction splined between [{:6.2f}:{:6.2f}] {} using {} knots w. "
                         "maximum absolute error of {:.1E} kT",
                         Faunus::atoms[i].name, Faunus::atoms[j].name, rmin, rmax,
                         unicode::angstrom, knotdata.numKnots(), max_error);
}
...
TEST_CASE("[Faunus] NewCoulombGalore")
{
    ...
    CHECK_EQ(force_on_a.z(),
             doctest::Approx(0.1429734149)); // attraction -> positive direction expected
}
...
void NewCoulombGalore::from_json(const json& j)
{
    ...
    if (method == "yukawa") {
        if (json _j(j); _j.value("shift", false)) { // zero energy and force at cutoff
            faunus_logger->debug(
                "energy and force shifted yukawa uses the 'poisson' scheme with C=1 and D=1");
            _j["type"] = "poisson";
            _j["C"] = 1;
            _j["D"] = 1;
            ...
            if (_j.contains("cutoff")) {
                throw ConfigurationError(
                    "unexpected 'cutoff' for non-shifted yukawa which is always infinity");
            }
            _j["type"] = "plain";
            _j["debyelength"] = electrolyte.value().debyeLength(bjerrum_length);
    ...
    faunus_logger->info("splitting function for '{}' splined with {} knots", method,
                        pot.numKnots().at(0));
    setSelfEnergy();
}
...
const CoulombGalore::Splined& NewCoulombGalore::getCoulombGalore() const
{
    return pot;
}

[[maybe_unused]] double NewCoulombGalore::dielectric_constant(double M2V)
{
    return pot.calc_dielectric(M2V);
}

// =============== Multipole ===============

Multipole::Multipole(const std::string& name)
    : NewCoulombGalore(name)
{
...
TEST_CASE("[Faunus] Dipole-dipole interactions")
{
    ...
    auto dipoledipole =
        pairpotential::makePairPotential<Multipole>(R"({"epsr": 1.0, "type": "plain"})"_json);
    ...
    CHECK_EQ(u(a, a, r2, r), Approx(dipoledipole(a, a, r2,
                                                 r))); // interaction between two parallell dipoles,
                                                       // directed parallell to their seperation
    CHECK_EQ(u(b, b, r2, r),
             Approx(dipoledipole(b, b, r2, r))); // interaction between two parallell dipoles,
                                                 // directed perpendicular to their seperation
    CHECK_EQ(u(a, b, r2, r),
             Approx(dipoledipole(a, b, r2, r))); // interaction between two perpendicular dipoles
...
TEST_CASE("[Faunus] WeeksChandlerAndersen")
{
    SUBCASE("JSON initilization")
    {
        ...
        CHECK_THROWS_AS(makePairPotential<WeeksChandlerAndersen>(R"({"mixing": "unknown"})"_json),
                        std::runtime_error);
        ...
        {
            auto wca = makePairPotential<WeeksChandlerAndersen>(
                R"({"mixing": "LB", "sigma": "sigma_wca"})"_json);
    ...
    SUBCASE("JSON initilization custom coefficient names")
    {
        ...
        // CHECK_THROWS_AS_MESSAGE(WeeksChandlerAndersen wca = R"({"mixing": "LB"})"_json,
        // std::runtime_error,
        //                         "unknown atom property");
        // CHECK_THROWS_AS_MESSAGE(WeeksChandlerAndersen wca = R"({"mixing": "LB", "sigma":
        // "unknown"})"_json,
        ...
        //         R"({"mixing": "LB", "sigma": "sigma_wca", "custom": [{"A B": {"eps": 0.5,
        //         "sigma": 8}}]})"_json,
        //     std::runtime_error, "unknown atom property");
    }
    SUBCASE("JSON serialization")
...
void CosAttractMixed::initPairMatrices()
{
    ...
    switching_distance = PairMixer(extract_rc, comb_distance, &PairMixer::modIdentity)
                             .createPairMatrix(atoms, *custom_pairs);
    switching_width = PairMixer(extract_wc, comb_distance, &PairMixer::modIdentity)
                          .createPairMatrix(atoms, *custom_pairs);
    epsilon = PairMixer(extract_eps, comb_epsilon, &PairMixer::modIdentity)
                  .createPairMatrix(atoms, *custom_pairs);

    faunus_logger->trace(
        "Pair matrices for {} sigma ({}×{}) and epsilon ({}×{}) created using {} custom pairs.",
        name, switching_distance->rows(), switching_distance->cols(), epsilon->rows(),
        epsilon->cols(), custom_pairs->size());
}
...
CosAttractMixed::CosAttractMixed(const std::string& name, const std::string& cite,
                                 CombinationRuleType combination_rule)
    : MixerPairPotentialBase(name, cite, combination_rule)
{
}
