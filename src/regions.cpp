...
std::optional<double> RegionBase::volume() const
{
    return std::nullopt;
}

bool RegionBase::inside(const Particle& particle) const
{
    return isInside(particle.pos);
}
 ...
 * @throw if `use_mass_center==true` and the given molecule type is not molecular, i.e. has
 * ill-defined mass center
 */
WithinMoleculeType::WithinMoleculeType(const Space& spc, std::string_view molecule_name,
                                       double threshold, bool use_region_mass_center,
    ...
    , threshold_squared(threshold * threshold)
{
    ...
    RegionBase::use_group_mass_center =
        use_group_mass_center; // use mass-center of groups to be tested?
}
...
bool WithinMoleculeType::isInside(const Point& position) const
{
    ...
    auto has_position_inside = [&](const Group& group) {
        ...
        return any_of(group.positions(), [&](const auto& position_in_group) {
            return within_threshold(position, position_in_group);
        });
 ...
 */
SphereAroundParticle::SphereAroundParticle(const Space& spc, ParticleVector::size_type index,
                                           double radius)
...
bool MovingEllipsoid::isInside(const Point& position) const
{
    ...
    const auto midpoint_pos_len =
        midpoint_pos.norm() + pc::epsilon_dbl; // must not be *exactly* zero
    ...
    const auto coord = x * x / parallel_radius_squared +
                       y * y / (perpendicular_radius * perpendicular_radius); // normalized coord
...
std::pair<Point, Point> MovingEllipsoid::getEllipsoidPositionAndDirection() const
{
    ...
    if (parallel_radius < distance) { // is this check needed?
        faunus_logger->error(
            "Parallel radius ({} Å) smaller than half distance between reference atoms ({} Å)",
...
MovingEllipsoid::MovingEllipsoid(const Space& spc, const json& j)
    : MovingEllipsoid(spc, j.at("index1").get<int>(), j.at("index2").get<int>(),
                      j.at("parallel_radius").get<double>(),
